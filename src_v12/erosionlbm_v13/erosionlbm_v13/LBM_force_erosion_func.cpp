		...
		for (iy = 0; iy < Ny; iy++) {
			for (ix = 0; ix < Nx; ix++) {
				...
				if (solid_list(ix, iy, iz) == 0) { // solid surface point
					...
					//anstemp = shearforce[0] + shearforce[1] + shearforce[2] - pow(tau_stress(ix,iy,iz,0)*nhat(ix,iy,iz,0), 2) - pow(tau_stress(ix, iy, iz, 1)*nhat(ix, iy, iz, 1), 2) - pow(tau_stress(ix, iy, iz, 2)*nhat(ix, iy, iz, 2), 2);
					//if (anstemp < 0)
					//	anstemp = 0.;
					//F_sum(ix, iy, iz, 0) = sqrt(anstemp);
